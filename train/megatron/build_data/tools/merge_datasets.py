...
def main(args):
    ...
    for basename in os.listdir(args.input):
        ...
        if prefix in prefixes or ext != '.idx':
            continue
    ...
    builder = None
    for prefix in tqdm(sorted(prefixes)):
        if builder is None:
            dataset = indexed_dataset.make_dataset(os.path.join(args.input, prefix), 'infer', skip_warmup=True)
