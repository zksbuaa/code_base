...
def get_embedding(text: list[str], tokenizer, model, max_seq_len) -> np.ndarray:
    ...
    truncated_tokens = [inputs[i][:max_seq_len] for i in range(len(inputs))]
    truncated_texts = tokenizer.batch_decode(truncated_tokens, skip_special_tokens=True)

    outputs = model.embed(truncated_texts)

    return np.array([output.outputs.embedding for output in outputs], dtype=np.float32)
...
if __name__ == "__main__":
    ...
    emb_model = LLM(**model_args)
    emb_tokenizer = emb_model.get_tokenizer()
